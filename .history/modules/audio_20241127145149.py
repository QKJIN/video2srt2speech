        ...
        for batch_index in range(0, len(subtitles), BATCH_SIZE):
            ...
            try:
                ...
                for i, subtitle in enumerate(batch_subtitles):
                    ...
                    if audio_file.exists():
                        ...
                        filter_parts.append(
                            ...
                            f'apad=whole_dur={batch_duration}' +  # 填充到批次总长度
                ...
                if valid_audio_count > 0:
                    ...
                    # 添加基础静音轨道
                    filter_complex += '[0:a]apad=whole_dur={batch_duration}[base];'
                    
                    # 构建混音命令
                    mix_parts = ['[base]']
                    for i in range(valid_audio_count):
                        mix_parts.append(f'[delayed{i}]')
                    filter_complex += ''.join(mix_parts) + f'amix=inputs={valid_audio_count + 1}:dropout_transition=0[aout]'
