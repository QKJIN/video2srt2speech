...
async def merge_audio(file_id: str, target_language: str):
    try:
        ...
        for batch_index in range(0, len(subtitles), BATCH_SIZE):
            ...
            print(f"处理批次 {batch_index//BATCH_SIZE + 1}: {batch_start:.2f}s - {batch_end:.2f}s")
        ...
        if temp_files:
            # 创建最终的静音基础
            final_base = TEMP_DIR / f"{file_id}_final_base.wav"
            subprocess.run([
                'ffmpeg', '-y',
                '-f', 'lavfi',
                '-i', f'anullsrc=r=16000:cl=mono',
            ...
            try:
                # 准备最终合并的过滤器
                final_inputs = ['-i', str(final_base)]
                final_filter_parts = []
                
                for i, (temp_file, start_time) in enumerate(temp_files):
                    final_inputs.extend(['-i', str(temp_file)])
                    final_filter_parts.append(
                        f'[{i+1}:a]adelay={int(start_time*1000)}|{int(start_time*1000)}[temp{i}];'
                    )

                # 构建最终混音命令
                final_filter = ''.join(final_filter_parts)
                final_filter += '[0:a]volume=0.1[base];[base]'
                for i in range(len(temp_files)):
                    final_filter += f'[temp{i}]'
                final_filter += f'amerge=inputs={len(temp_files) + 1}[aout]'
                ...
                # 清理临时文件
                final_base.unlink(missing_ok=True)
