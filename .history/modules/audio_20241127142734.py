...
async def merge_audio(file_id: str, target_language: str):
    try:
        ...
        # 创建一个临时文件列表
        temp_list_file = TEMP_DIR / f"{file_id}_audio_list.txt"
        ...
        try:
            ...
            with open(temp_list_file, 'w', encoding='utf-8') as f:
                for i, subtitle in enumerate(subtitles):
                    ...
                    # 如果当前时间和下一个字幕开始时间有间隔，添加静音
                    if start_time > current_time:
                        silence_duration = start_time - current_time
                        silence_file = TEMP_DIR / f"{file_id}_silence_{i}.wav"
                    ...
                    if audio_file.exists():
                        ...
                        current_time = start_time + float(subtitle['duration'])
            
            print("临时文件列表内容:")
            with open(temp_list_file, 'r') as f:
                print(f.read())
            
            # 使用 ffmpeg 的 concat demuxer 合并所有文件
            cmd = [
                'ffmpeg', '-y',
                '-f', 'concat',
                '-safe', '0',
                '-i', str(temp_list_file.absolute()),
                '-c:a', 'libmp3lame',
                '-q:a', '4',
                str(output_path)
            ]
            ...
            # 清理临时静音文件
            for temp_file in TEMP_DIR.glob(f"{file_id}_silence_*.wav"):
                temp_file.unlink()
