    ...
    try:
        ...
        for i, subtitle in enumerate(subtitles):
            ...
            if audio_file.exists():
                ...
                # 修改这里：将音频音量调高，并添加淡入淡出效果
                filter_parts.append(
                    f'[{valid_audio_count + 1}:a]volume=1.5,afade=t=in:st={start_time}:d=0.5,afade=t=out:st={start_time + float(subtitle["duration"]) - 0.5}:d=0.5,adelay={int(start_time*1000)}|{int(start_time*1000)}[delayed{valid_audio_count}];'
                )
                valid_audio_count += 1
        ...
        if valid_audio_count > 0:
            ...
            # 然后添加混音命令，修改这里：调整混音参数
            filter_complex += '[0:a]volume=0.1[base];[base]'  # 降低背景静音的音量
            for i in range(valid_audio_count):
                filter_complex += f'[delayed{i}]'
            # 使用 amerge 而不是 amix，以保持原始音量
