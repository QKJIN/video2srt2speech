    ...
    try:
        ...
        for i, subtitle in enumerate(subtitles):
            ...
            if audio_file.exists():
                ...
                filter_parts.append(
                    f'[{valid_audio_count + 1}:a]adelay={int(start_time*1000)}|{int(start_time*1000)}[delayed{valid_audio_count}];'
                )
                valid_audio_count += 1
        ...
        if valid_audio_count > 0:
            ...
            # 然后添加混音命令
            filter_complex += '[0:a]'  # 基础静音轨道
            for i in range(valid_audio_count):
                filter_complex += f'[delayed{i}]'
            filter_complex += f'amix={valid_audio_count + 1}:duration=first:dropout_transition=0[aout]'
