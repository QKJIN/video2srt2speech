    ...
    try:
        ...
        for i, subtitle in enumerate(subtitles):
            try:
                ...
                if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
                    # 计算需要添加的静音时长
                    current_start = subtitle.get("start", 0)
                    current_duration = subtitle.get("duration", 0)
                    next_start = subtitles[i + 1].get("start", 0) if i < len(subtitles) - 1 else current_start + current_duration
                    silence_duration = next_start - (current_start + current_duration)
                    ...
                    audio_files.append({
                        ...
                        "text": text,
                        "start": current_start,
                        "duration": current_duration + silence_duration  # 包含静音时长
                    })
                else:
                    error_details = result.properties.get(
                        speechsdk.PropertyId.SpeechServiceResponse_JsonErrorDetails
