            ...
            try:
                ...
                for i, subtitle in enumerate(batch_subtitles):
                    ...
                    if audio_file.exists():
                        ...
                        # 修改过滤器，确保音频在正确的时间点播放
                        filter_parts.append(
                            f'[{valid_audio_count + 1}:a]volume=1.5,' +  # 调整音量
                            f'apad=pad_dur={batch_duration},' +  # 填充到批次总长度
                            f'adelay={int(relative_start*1000)}|{int(relative_start*1000)},' +  # 延迟到正确位置
                            f'volume=enable=\'between(t,{relative_start},{relative_start+duration})\':volume=1:volume=0,' +  # 只在指定时间段播放
                ...
                if valid_audio_count > 0:
                    ...
                    # 基础静音轨道
                    filter_complex += '[0:a]volume=0.1[base];'
                    # 合并所有音轨
                    filter_complex += '[base]'
                    for i in range(valid_audio_count):
                        filter_complex += f'[delayed{i}]'
                    filter_complex += f'amix=inputs={valid_audio_count + 1}:duration=first[aout]'
