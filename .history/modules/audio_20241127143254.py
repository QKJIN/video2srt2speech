        ...
        for batch_index in range(0, len(subtitles), BATCH_SIZE):
            ...
            for i, subtitle in enumerate(batch_subtitles):
                ...
                if audio_file.exists():
                    ...
                    filter_parts.append(
                        f'[{valid_audio_count + 1}:a]volume=1.5,afade=t=in:st={relative_start}:d=0.5,afade=t=out:st={relative_start + float(subtitle["duration"]) - 0.5}:d=0.5,adelay={int(relative_start*1000)}|{int(relative_start*1000)}[delayed{valid_audio_count}];'
                    )
                    valid_audio_count += 1
