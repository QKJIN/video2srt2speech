...
async def extract_audio_segment(audio_path: Path, output_path: Path, start_time: float, end_time: float):
    ...
    try:
        ...
        # 验证音频格式
        format_check = await asyncio.create_subprocess_exec(
            'ffprobe',
            '-v', 'error',
            '-select_streams', 'a:0',
        ...
        format_stdout, format_stderr = await format_check.communicate()
        if format_check.returncode == 0:
            codec, sample_rate, channels = format_stdout.decode().strip().split('\n')
            print(f"音频格式验证: codec={codec}, sample_rate={sample_rate}, channels={channels}")
            
            # 确保格式正确
            if codec != 'pcm_s16le' or sample_rate != '16000' or channels != '1':
                raise HTTPException(500, "音频格式不符合要求")
