...
async def extract_audio_segment(audio_path: Path, output_path: Path, start_time: float, end_time: float):
    ...
    try:
        ...
        try:
            ...
            stdout, stderr = await process.communicate()
            if process.returncode == 0:
                format_info = stdout.decode().strip().split('\n')
                print(f"音频格式: {format_info}")
                
                if len(format_info) >= 3:
                    codec, sample_rate, channels = format_info[:3]
                    if not (codec == 'pcm_s16le' and sample_rate == '16000' and channels == '1'):
                        raise Exception(f"音频格式不符合要求: codec={codec}, sample_rate={sample_rate}, channels={channels}")
